...
# importing DQN and logger tools
...
run = wandb.init(
    project="atari_dqn_assault",
    config = {"policy": "DQN_MlpPolicy", "learning_rate": 0.000827, "gamma":0.96979, "gae_lambda":0.92680, "ent_coef":.0000199},
    monitor_gym=True,
...
model = DQN("MlpPolicy", env, learning_rate= 0.000827, gamma = 0.96979, gae_lambda= 0.92680, ent_coef= 0.0000199, verbose=1, tensorboard_log=f"runs/dqn_assault")
...
# Save the agent
model.save("atari_dqn_assault")
vec_env = model.get_env()

del model # remove to demonstrate saving and loading

# Load the agent
model = PPO.load("atari_dqn_assault")
...
# establish new environment for Test
new_env = DummyVecEnv([make_env_test])
new_env = VecVideoRecorder(new_env, f"videos/{run.id}", record_video_trigger = lambda x: x % 2000 == 0, video_length=200) 

# Evaluate the agent performance from Test
